...
<body class="gray-bg">
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div style="margin-bottom: 8px">
							<a href="<%=path%>/empservlet?state=toadd" class="btn btn-success">添加员工</a>
						</div>
							...
							<tbody>
							 <c:forEach items="${list}" var="m" varStatus="i">
								<tr class="gradeA">
									...
									<td>${m.in_time }</td>
									<td><a href="<%=path %>/empservlet?state=show&id=${m.id}" class="btn btn-info">查看</a>&nbsp;&nbsp;
										<a href="<%=path %>/empservlet?state=update&id=${m.id}" class="btn btn-primary">修改</a>&nbsp;&nbsp;
							...
							<nav aria-label="Page navigation" style="margin: 0 auto; width: 320px">
								<ul class="pagination" style="margin: 0;">
									<c:if test="${pe.pageIndex!=1 }">
											...
											<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=1" aria-label="Previous"> <span aria-hidden="true">首页</span></a></li>
											<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=${pe.pageIndex-1 }" aria-label="Previous"> <span aria-hidden="true">前一页</span></a></li>
										
									</c:if>
									<c:forEach begin="1" end="${pe.pageCount }" var="i">
										
											<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=${i }" ${i==pe.pageIndex?"style=\"color:#ff2200\";":""} class="a">${i }</a></li>
										
									</c:forEach>
									<c:if test="${pe.pageIndex!=pe.pageCount&&pe.pageCount!=0 }">
											<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=${pe.pageIndex+1 }" aria-label="Next"> <span aria-hidden="true">后一页</span></a></li>
											<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=${pe.pageCount }" aria-label="Next"> <span aria-hidden="true">尾页</span></a></li>
									</c:if>
								</ul>
								<%-- <ul class="pagination" style="margin: 0;">
									<li>
								...
								%>
									<li><a href="<%=path%>/empservlet?state=findallbypage&pageIndex=<%=i%>"><%=i %></a></li>
								<%
	 								 }
								%>
	...
	function del(id){
		parent.layer.confirm('确认删除？', {
		    ...
		    location.href="./"+"/empservlet?state=delete&id="+id;
		});
	}
	</script>
</body>
